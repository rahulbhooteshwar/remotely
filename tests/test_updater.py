...
def test_apply_replaces_the_running_binary(tmp_path, monkeypatch, fake_release) -> None:
    ...
    assert installed.stat().st_mode & 0o111, "new binary is not executable"
    # No staging file left behind.
    assert not (installed.parent / "remotely.new").exists()
...
def test_apply_refuses_a_bad_checksum(tmp_path, monkeypatch, fake_release) -> None:
    ...
    assert installed.read_bytes() == b"old binary", "binary was replaced despite a bad checksum"
